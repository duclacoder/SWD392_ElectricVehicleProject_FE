...
import type { AuctionCustom, CombinedData } from "../../entities/Auction.ts";
...
const AuctionPage: React.FC = () => {
  ...
  const PAGE_SIZE = 50; // Tăng pageSize để tải đủ dữ liệu cho một trang
  ...
  const fetchVehiclesForAuctions = useCallback(
    async (fetchedAuctions: AuctionCustom[]) => {
      try {
        const vehicleIds = fetchedAuctions.map((auction) => auction.vehicleId);
  ...
  const sortedData = useMemo(() => {
    return [...filteredData].sort((a, b) => {
      ...
      switch (sortBy) {
        ...
        default:
          return (
            new Date(b.auction.startTime).getTime() -
            new Date(a.auction.startTime).getTime()
          );
      }
    });
  }, [filteredData, sortBy]);
  ...
  const activeAuctionsCount = useMemo(
    () =>
      combinedData.filter(
        (item) => item.auction.status.toLowerCase() === "active"
      ).length,
    [combinedData]
  );
  ...
  const totalBids = useMemo(
    () =>
      combinedData.reduce(
        (sum, item) => sum + (item.auction.bids?.length || 0),
        0
      ),
    [combinedData]
  );
  ...
  const availableBrands = useMemo(
    () =>
      Array.from(new Set(combinedData.map((item) => item.vehicle.brand)))
        .filter((brand) => brand)
  ...
  const availableYears = useMemo(
    () =>
      Array.from(new Set(combinedData.map((item) => item.vehicle.year)))
        .filter((year) => year)
        .sort((a, b) => b - a),
    [combinedData]
  );
    ...
    <>
      ...
      <section className="relative bg-gradient-to-br from-blue-700 to-indigo-800 text-white py-20 overflow-hidden shadow-xl">
        ...
        <div className="relative max-w-7xl mx-auto px-6 text-center z-10">
          <h1 className="text-5xl md:text-6xl font-extrabold mb-4 animate-fade-in-down">
            <span className="block text-blue-200 text-3xl mb-2">
              Chào mừng đến với
            </span>
          ...
          <p className="text-xl md:text-2xl text-blue-100 mb-10 max-w-4xl mx-auto leading-relaxed animate-fade-in-up">
            Nơi những giấc mơ xe hơi của bạn trở thành hiện thực với các phiên
            đấu giá công bằng và minh bạch.
          </p>
      ...
      <main className="max-w-7xl mx-auto px-6 py-12 bg-white min-h-[60vh]">
        {sortedData.length === 0 ? (
          ...
          <>
            {/* Truyen du lieu da sap xep va loc vao AuctionCard */}
            <AuctionCard auctions={sortedData} />

            {/* Load More (placeholder) */}
            {sortedData.length >= PAGE_SIZE && ( // Chỉ hiển thị nếu có thể có thêm kết quả
